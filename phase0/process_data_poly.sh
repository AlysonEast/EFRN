...
IMPORT_POLYGON=0
RASTERIZE_FORESTS=0
EXTRACT_DATA_AT_EF=0
RESTRUCTURE_UNIVAR=0
CALCULATE_REPRESENTATIVENESS=0
IMPORT_REPRESENTATIVENESS_MAPS=1
IMPORT_CONSTITUENCY_MAPS=1
...
then
 ...
 CASE=northamerica_forcasts_phase0

 r.stats -1gn input=wc2.0_bio_30s_01_ann_temp,wc2.0_bio_30s_02_diurnal_range,wc2.0_bio_30s_03_isothermality,wc2.0_bio_30s_04_temp_seasonality,wc2.0_bio_30s_10_meant_warmqtr,wc2.0_bio_30s_11_meant_coldqtr,wc2.0_bio_30s_12_ann_precip,wc2.0_bio_30s_15_precip_seasonality,wc2.0_bio_30s_16_precip_dry_qtr,wc2.0_bio_30s_17_precip_wet_qtr,wc2.0_bio_30s_18_precip_warm_qtr,wc2.0_bio_30s_19_precip_cold_qtr,available_water_capacity_until_wilting_point_0-100cm_30sec,bulk_density_0-100cm_mean_30sec,nitrogen_0-100cm_mean_30sec,ph_0-100cm_mean_30sec,soil_organic_carbon_0-100cm_mean_30sec >data.out
...
then
 ...
 RCASE=northamerica_forcasts_phase0
 CASE=exp_forests
...
awk 'BEGIN { FS="|"; }
 {print $8" "$16" "$24" "$32" "$40" "$48" "$56" "$64" "$72" "$80" "$88" "$96" "$104" "$112" "$120" "$128" "$136}' univar.vect.out > ${CASE}.mean.vect.out
...
# col=${ncol}-1
 r.mask -r
awk '{print $1" "$2" "$(NF-18)}' ../representativeness/${CASE}.representativeness >temp
...
#Last number is number of sites +4
 ...
 r.mask -r
 awk '{print $1" "$2" "$(NF-17)}' ../representativeness/${CASE}.representativeness >temp
