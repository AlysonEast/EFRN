...
png(file="../figures/drivers_poorrep_region_hist.png", width=11, height=6.5, units = "in", res=300) 
ggplot(data=subset(driver_netrep, poor_rep==1), aes(fill=Catagory, x=max)) + 
  geom_histogram(binwidth=1, alpha=0.8)+ 
  scale_fill_manual(values = c("#3291c5","#977552","#65b12a","#f9634a"))+
  facet_wrap(.~regionName, scales = "free_y") + theme_bw() +
...
driver_vars$driver_vars<-ifelse(driver_vars$region==1, paste0("NRS"),
                                 ifelse(driver_vars$region==2, paste0("PNW"),
                                        ifelse(driver_vars$region==3, paste0("PSW"),
                                               ifelse(driver_vars$region==4, paste0("RMRS"), paste0("SRS")))))
...
driver_vars_sub<-subset(driver_vars, driver_vars=="NRS" & variable=="MeanAnnTemp" |
                          ...
                          driver_vars=="RMRS" & variable=="WaterCapacity" |
                          driver_vars=="SRS" & variable=="TempWarmQtr" |
                          driver_vars=="SRS" & variable=="WaterCapacity" |
                          driver_vars=="SRS" & variable=="Nitrogen" )
...
ggplot(data=driver_vars_sub, alpha=0.55, aes(fill=in_ef, col=in_ef, x=value)) + 
  ...
  scale_fill_manual(values = c("#138a2c","#88422F"), labels = c("Outside Experimental Forests","In Experimental Forests"))+
